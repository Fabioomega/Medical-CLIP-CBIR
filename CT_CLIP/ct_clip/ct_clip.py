...
def l2norm(t):
    return F.normalize(t, dim=-1) if not t is None else None
...
class CTCLIP(nn.Module):
    ...
    def forward(
        ...
        if not self.text_transformer is None:
        ...
        if self.text_causal_mask and not self.text_transformer is None:
        ...
        if self.use_all_token_embeds and not enc_text is None:
            assert (
                enc_text.ndim == 3
